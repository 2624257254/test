...
uint8_t dht11_check(void)
{
    ...
    while (DHT11_IN && retry < 100)         /* DHT11会拉低大约83us */
    {
        retry++;
        delay_us(1);
    }
    ...
    else 
    {
        ...
        while (!DHT11_IN && retry < 100)    /* DHT11拉低后会拉高约87us */
        {
            retry++;
            delay_us(1);
        }
...
uint8_t dht11_read_data(uint8_t *temp, uint8_t *humi)
{
    ...
    if (dht11_check() == 0)     /* dht11正常，开始读取40位数据*/
    {
        for (i = 0; i < 5; i++)
        {
...
/**
 * @brief       初始化DHT11的IO口，同时检测DHT11的存在
 * @param       无
 * @retval      0, 正常
 *              1, 不存在/不正常
 */
uint8_t dht11_init(void)
{
    ...
    DHT11_GPIO_CLK_ENABLE();     /* 开启DQ引脚时钟 */

    gpio_init_struct.Pin = DHT11_GPIO_PIN;
    gpio_init_struct.Mode = GPIO_MODE_OUTPUT_OD;            /* 开漏输出 */
    gpio_init_struct.Pull = GPIO_PULLUP;                    /* 上拉 */
    gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;          /* 高速 */
    HAL_GPIO_Init(DHT11_GPIO_PORT, &gpio_init_struct);   /* 初始化DHT11_DQ引脚 */
