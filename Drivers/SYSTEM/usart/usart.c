...
uint8_t g_usart_rx_recv_buf[USART_REC_LEN];
...
uint16_t g_usart_rx_recv_sta = 0;
...
void usart_init(uint32_t baudrate)
{
    ...
    __HAL_USART_CLEAR_IDLEFLAG(&g_uart3_handle);                                /* 清除空闲中断 */
    __HAL_USART_ENABLE_IT(&g_uart3_handle, USART_IT_IDLE);                      /* 开启空闲中断 */
...
void USART_UX_IRQHandler(void)
{
    ...
    if (__HAL_USART_GET_FLAG(&g_uart3_handle, USART_FLAG_IDLE ) != RESET)
    {
        __HAL_USART_CLEAR_IDLEFLAG(&g_uart3_handle);
        if (strstr((const char*)g_usart_rx_buf, "MQTTSUBRECV") != NULL)              /* 检查是否是接收到了订阅消息 */
        {
            g_usart_rx_recv_sta |= 0x8000;
        }
        else{
